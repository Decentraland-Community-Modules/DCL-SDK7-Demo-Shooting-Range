...
export module ScoreObject {
	...
	export function Create(data:ScoreDataObject):Entity {
		if(isDebugging) console.log("Score Object: attempting to create object of type="+data.type+" at pos(x="+data.pos.x+", y="+data.pos.y+", z="+data.pos.z+")...");
		...
		for (let i = 0; i < pooledObjects[data.type].length; i++) {
			if(!ScoreComponent.get(pooledObjects[data.type][i]).isActive) {
				if(isDebugging) console.log("Score Object: recycled unused object (ID="+pooledObjects[data.type][i].toString()+")!");
        ...
        //add animator
	 	Animator.create(entity, {
            states:[
                { name: 'Pop', clip: 'Pop', playing: true, loop: false, shouldReset: true },
            ]
        });
		//  add component
