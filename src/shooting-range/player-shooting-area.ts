...
import { AudioSource, AvatarAnchorPointType, AvatarAttach, Entity, GltfContainer, InputAction, Material, MeshCollider, MeshRenderer, PointerEventType, PointerEvents, RaycastQueryType, Transform, engine, inputSystem, raycastSystem } from "@dcl/sdk/ecs";
...
export module PlayerShootingArea {
    ...
    const AUDIO_SHOT_SFX:string[] = [
        'audio/shooting-range/shot-hit.mp3',
        'audio/shooting-range/shot-missed.mp3'
    ];
            ...
            function (raycastResult) {
                ...
                if (raycastResult.hits.length > 0) {
                    if (raycastResult.hits[0].entityId) {
                        ...
                        if(isDebugging) console.log("Shooting Area: hit validated entityID="+hitID+", mesh="+ raycastResult.hits[0].meshName+
                        ...
                        switch(raycastResult.hits[0].meshName) {
                            case 'target10_collider': 
                                ScoreObject.Create({type:ScoreObject.SCORE_TYPE.TEN, pos:{x:hitPos.x,y:hitPos.y,z:hitPos.z-0.5}});
                            break;
                            case 'target25_collider': 
                                ScoreObject.Create({type:ScoreObject.SCORE_TYPE.TWENTYFIVE, pos:{x:hitPos.x,y:hitPos.y,z:hitPos.z-0.5}});
                            break;
                            case 'target50_collider': 
                                ScoreObject.Create({type:ScoreObject.SCORE_TYPE.FIFTY, pos:{x:hitPos.x,y:hitPos.y,z:hitPos.z-0.5}});
                            break;
                        }
                        //play audio - hit
                        PlayShotSound(0);
